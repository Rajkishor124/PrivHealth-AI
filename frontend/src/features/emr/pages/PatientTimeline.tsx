...
import { ArrowLeft, Loader2, Stethoscope, Pill, FileText, FileBarChart, AlertCircle, Calendar, HeartPulse, Activity } from 'lucide-react';
...
export default function PatientTimeline() {
  ...
  useEffect(() => {
    const fetchTimeline = async () => {
      try {
        setLoading(true);
        const data = await trackingApi.getUnifiedTimeline(patientId);
        setTimeline(data);
      } catch (error) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'APPOINTMENT': return <div className="p-2 bg-teal-100 text-teal-600 rounded-full"><Calendar size={20} /></div>;
      case 'SYMPTOM': return <div className="p-2 bg-red-100 text-red-600 rounded-full"><Activity size={20} /></div>;
      case 'VITALS': return <div className="p-2 bg-rose-100 text-rose-600 rounded-full"><HeartPulse size={20} /></div>;
      case 'JOURNAL': return <div className="p-2 bg-yellow-100 text-yellow-600 rounded-full"><FileText size={20} /></div>;
      default: return <div className="p-2 bg-slate-100 text-slate-600 rounded-full"><FileText size={20} /></div>;
    }
  };
